...
function SubscriptionPage() {
  ...
  const { data: subs, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("subscriptions")
        .select("id,status,started_at,expires_at,plan:plans(name,duration_days,price_kes)")
        .eq("user_id", user!.id)
  ...
  const { data: payments } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("payments")
        .select("id,amount,currency,status,tx_ref,created_at,plan:plans(name)")
        .eq("user_id", user!.id)
  ...
  const now = Date.now();
  const active = (subs ?? []).find((s: any) => s.status === "active" && new Date(s.expires_at).getTime() > now);

  return (
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-[color:var(--navy-deep)]">
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border border-primary/40 bg-primary/10 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold text-lg">{(active as any).plan?.name ?? "VIP"} — Active</div>
                  <div className="text-sm text-muted-foreground">
                    Expires {new Date((active as any).expires_at).toLocaleString()}
                  </div>
                </div>
              </div>
            </div>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { cls: string; Icon: any }> = {
    active: { cls: "bg-green-500/15 text-green-400 border-green-500/30", Icon: CheckCircle2 },
    success: { cls: "bg-green-500/15 text-green-400 border-green-500/30", Icon: CheckCircle2 },
    completed: { cls: "bg-green-500/15 text-green-400 border-green-500/30", Icon: CheckCircle2 },
    pending: { cls: "bg-yellow-500/15 text-yellow-400 border-yellow-500/30", Icon: Clock },
    expired: { cls: "bg-muted text-muted-foreground border-border", Icon: XCircle },
