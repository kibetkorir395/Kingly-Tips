...
type TipForm = {
  match_date: string; kickoff_time: string; league: string;
...
function AdminPage() {
  ...
  const [tab, setTab] = useState<"free" | "vip" | "plans">("free");
...
function TipsAdmin({ table, maxFree }: { table: "free_tips" | "vip_tips"; maxFree?: boolean }) {
  const qc = useQueryClient();
  const [form, setForm] = useState<TipForm>(emptyTip);
  ...
  async function add(e: React.FormEvent) {
    ...
    if (maxFree) {
      ...
      const upcoming = (tips ?? []).filter((t: any) => t.match_date >= today);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="mb-3 font-display text-lg font-bold uppercase">Add {table === "free_tips" ? "Free" : "VIP"} Tip {maxFree && <span className="text-xs font-normal text-muted-foreground">(max 4 upcoming)</span>}</h3>
...
function EditableRow({ tip, onSave, onDelete }: { tip: any; onSave: (id: string, patch: any) => Promise<boolean>; onDelete: () => void }) {
  const [edit, setEdit] = useState(false);
  const [row, setRow] = useState<any>(tip);
